...
print('-----------------------------------------------------------')
#Experimenting with histogram columns

total = 0
length = 0
myList = []
for n in range (len(data)):
    metallicity = data[n][col['stMt']]
    if metallicity != '':
        myList.append(float(metallicity))
        total += float(metallicity)
        length += 1
...
for n in range(len(myList)):
    third = len(myList) / 3
    if n <= third: 
        lowList.append(myList[n])
    elif n > third and n <= (third*2):
        medList.append(myList[n])
    else:
        highList.append(myList[n])

print('Low value = ' + str(min(lowList)) + ' - ' + str(max(lowList)))
print('Medium value = ' + str(min(medList)) + ' - ' + str(max(medList)))
print('High value = ' + str(min(highList)) + ' - ' + str(max(highList)))
print('Length of entire list: ',str(len(myList)))
